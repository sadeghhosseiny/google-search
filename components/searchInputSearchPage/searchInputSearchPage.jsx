...
function SearchInputSearchPage({ searchVal, setSearchVal, scroll }) {
  ...
  return (
    <div className={`ml-11 mobile:ml-0 flex items-center rounded-full border ${scroll ? 'shadow-none px-3' : 'pl-2 pr-6 shadow-lg'} w-full max-w-2xl 1.5xl:max-w-lg`}>
      ...
      <XIcon onClick={() => setSearchVal('')} className={`mr-4 ${scroll && 'h-4'} mobile:hidden h-6 text-gray-500 border-r border-gray-300 pr-3 cursor-pointer`} />
      <div />
      <MicrophoneIcon className={`mr-4 text-blue-500 ${scroll ? 'h-4' : 'h-6'} mobile:hidden cursor-pointer`} />
